<?php
namespace Modelo\BD;
use Modelo\Base;

require_once __DIR__.'/../Base/AdministracionClass.php';
require_once __DIR__.'/../Base/AusenciaClass.php';
require_once __DIR__.'/../Base/CentroClass.php';
require_once __DIR__.'/../Base/ConvenioAusenciaClass.php';
require_once __DIR__.'/../Base/EmpresaClass.php';
require_once __DIR__.'/../Base/EstadoClass.php';
require_once __DIR__.'/../Base/FranjasClass.php';
require_once __DIR__.'/../Base/GerenciaClass.php';
require_once __DIR__.'/../Base/FestivoClass.php';
require_once __DIR__.'/../Base/HorarioParteClass.php';
require_once __DIR__.'/../Base/HoraConvenioClass.php';
require_once __DIR__.'/../Base/HorariosClass.php';
require_once __DIR__.'/../Base/HorariosFranjasClass.php';
require_once __DIR__.'/../Base/HorariosTrabajadoresClass.php';
require_once __DIR__.'/../Base/LoginClass.php';
require_once __DIR__.'/../Base/LogisticaClass.php';
require_once __DIR__.'/../Base/ParteLogisticaClass.php';
require_once __DIR__.'/../Base/ParteProduccionClass.php';
require_once __DIR__.'/../Base/ParteProducionTareaClass.php';
require_once __DIR__.'/../Base/ProduccionClass.php';
require_once __DIR__.'/../Base/TareaClass.php';
require_once __DIR__.'/../Base/TiposFranjasClass.php';
require_once __DIR__.'/../Base/TipoTareaClass.php';
require_once __DIR__.'/../Base/TrabajadorClass.php';
require_once __DIR__.'/../Base/TrabajadorAusenciaClass.php';
require_once __DIR__.'/../Base/VehiculoClass.php';
require_once __DIR__.'/../Base/ViajeClass.php';

require_once __DIR__ .'/../Base/HoraConvenioClass.php';


?>
